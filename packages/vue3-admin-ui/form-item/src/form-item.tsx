...
const App = defineComponent({
  ...
  setup(props) {
    const { slots, item, data, field, form, onlyRead } = unref(props)
    return () => {
      if (item.slot) {
        ...
        return ChildTag || isGroup ? (
          <Tag disabled={onlyRead} {...item.$attr} v-model={data[field]} {...item.$on}>
            {item.options?.map((child: { label: string; value: any; disabled: boolean }) => {
              return isGroup ? (
                <ChildTag
                  key={child.value}
                  label={child.label}
                  value={child.value}
                  disabled={child.disabled || onlyRead}
                ></ChildTag>
              ) : (
                <ChildTag
                  key={child.value}
          ...
          <Tag disabled={onlyRead} {...item.$attr} v-model={data[field]} {...item.$on}></Tag>
        )
      }
    }
  }
...
const getTag = (el: string): { Tag: any; ChildTag: any; isGroup: boolean } => {
  ...
  const ChildTag = getElement(match?.[2] ? match[1] : match?.[0] ? 'option' : '')
  return {
    Tag,
    ChildTag,
    isGroup: !!match?.[2]
